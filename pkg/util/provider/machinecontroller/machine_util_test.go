		...
		DescribeTable("##General Machine Health Reconciliation", func(data *data) {
			...
			Entry("Machine in InPlaceUpdating state with node label node.machine.sapcloud.io/update-result=successful should be marked InPlaceUpdateSuccessful", &data{
				setup: setup{
					...
					nodes: []*corev1.Node{
						newNode(1, map[string]string{v1alpha1.LabelKeyNodeUpdateResult: v1alpha1.LabelValueNodeUpdateSuccessful}, nil, &corev1.NodeSpec{}, &corev1.NodeStatus{Phase: corev1.NodeRunning, Conditions: nodeConditions(false, false, false, false, false)}),
					},
					targetMachineName: machineSet1Deploy1 + "-" + "0",
				},
				expect: expect{
			...
			Entry("Machine in InPlaceUpdating state with node label node.machine.sapcloud.io/update-result=failed should be marked InPlaceUpdateFailed", &data{
				setup: setup{
					...
					nodes: []*corev1.Node{
						newNode(1, map[string]string{v1alpha1.LabelKeyNodeUpdateResult: v1alpha1.LabelValueNodeUpdateFailed}, nil, &corev1.NodeSpec{}, &corev1.NodeStatus{Phase: corev1.NodeRunning, Conditions: nodeConditions(false, false, false, false, false)}),
					},
					targetMachineName: machineSet1Deploy1 + "-" + "0",
				},
				expect: expect{
		...
		DescribeTable("##table",
			func(data *data) {
				...
				if data.expect.node != nil {
					...
					Expect(getErr).To(BeNil())
					updatedNodeCondition := nodeops.GetCondition(updatedNode, v1alpha1.NodeInPlaceUpdate)
					expectedNodeCondition := nodeops.GetCondition(data.expect.node, v1alpha1.NodeInPlaceUpdate)
			...
			Entry("when node has LabelKeyNodeCandidateForUpdate", &data{
				setup: setup{
					...
					nodes: []*corev1.Node{
						newNode(1, map[string]string{v1alpha1.LabelKeyNodeCandidateForUpdate: "true"},
				...
				expect: expect{
					...
					node: &corev1.Node{
						ObjectMeta: metav1.ObjectMeta{
							Name: "node-0",
							Labels: map[string]string{
								v1alpha1.LabelKeyNodeCandidateForUpdate: "true",
							},
						},
			...
			Entry("when node has LabelKeyNodeSelectedForUpdate", &data{
				setup: setup{
					...
					nodes: []*corev1.Node{
						newNode(1, map[string]string{
							v1alpha1.LabelKeyNodeCandidateForUpdate: "true",
							v1alpha1.LabelKeyNodeSelectedForUpdate:  "true",
						},
							nil, &corev1.NodeSpec{}, &corev1.NodeStatus{Phase: corev1.NodeRunning,
								Conditions: append(nodeConditions(true, false, false, false, false),
									corev1.NodeCondition{
				...
				setup: setup{
					...
					nodes: []*corev1.Node{
						newNode(1, map[string]string{
							v1alpha1.LabelKeyNodeCandidateForUpdate: "true",
							v1alpha1.LabelKeyNodeSelectedForUpdate:  "true",
							v1alpha1.LabelKeyNodeUpdateResult:       "successful",
						},
							nil, &corev1.NodeSpec{}, &corev1.NodeStatus{Phase: corev1.NodeRunning,
								Conditions: append(nodeConditions(true, false, false, false, false),
									corev1.NodeCondition{
					...
					nodes: []*corev1.Node{
						newNode(1,
							map[string]string{
								v1alpha1.LabelKeyNodeCandidateForUpdate: "true",
								v1alpha1.LabelKeyNodeSelectedForUpdate:  "true",
								v1alpha1.LabelKeyNodeUpdateResult:       "failed",
							},
							map[string]string{v1alpha1.AnnotationKeyMachineUpdateFailedReason: "OS image not found"},
			...
			Entry("when node is not present", &data{
				setup: setup{
					machine: newMachine(
						&machinev1.MachineTemplateSpec{ObjectMeta: *newObjectMeta(&metav1.ObjectMeta{GenerateName: machineSet1Deploy1}, 0)},
						nil,
						nil, nil, map[string]string{machinev1.NodeLabelKey: "node-0-test", v1alpha1.LabelKeyNodeSelectedForUpdate: "true"}, true, metav1.Now()),
					node: newNode(1, nil, nil, &corev1.NodeSpec{}, &corev1.NodeStatus{}),
				},
				expect: expect{
					retryPeriod: machineutils.ShortRetry,
					err:         errors.New("nodes \"node-0-test\" not found"),
					node:        nil,
				},
			}),
			Entry("when node condition is nil", &data{
				setup: setup{
					machine: newMachine(
						&machinev1.MachineTemplateSpec{ObjectMeta: *newObjectMeta(&metav1.ObjectMeta{GenerateName: machineSet1Deploy1}, 0)},
						nil,
						nil, nil, map[string]string{machinev1.NodeLabelKey: "node-0", v1alpha1.LabelKeyNodeSelectedForUpdate: "true"}, true, metav1.Now()),
					node: newNode(1, nil, nil, &corev1.NodeSpec{}, &corev1.NodeStatus{}),
				},
				expect: expect{
					retryPeriod: machineutils.LongRetry,
