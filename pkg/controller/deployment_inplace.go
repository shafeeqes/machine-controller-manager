...
func (dc *controller) rolloutInPlace(ctx context.Context, d *v1alpha1.MachineDeployment, isList []*v1alpha1.MachineSet, machineMap map[types.UID]*v1alpha1.MachineList) error {
	...
	if len(oldISs) > 0 && !dc.machineSetsScaledToZero(oldISs) {
		...
		err := dc.labelMachineSets(ctx, oldISs, map[string]string{v1alpha1.LabelKeySkipMachineSetUpdate: "true"})
		if err != nil {
			klog.Errorf("Failed to add %s on all machine sets. Error: %s", v1alpha1.LabelKeySkipMachineSetUpdate, err)
			return err
		}
	}
